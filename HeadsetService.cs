...
{
    /// <summary>Battery percent dropped since the previous reading (direct byte read, or trend fallback).</summary>
    Discharging,
    /// <summary>Battery percent rose since the previous reading (direct byte read, or trend fallback).</summary>
    Charging,
    /// <summary>Battery available but direction not yet known (first reading, or byte value unrecognized).</summary>
    TrendPending,
    /// <summary>Dongle present but headset is off / out of range (HID connection byte = 0x02, or read timeout).</summary>
    HeadsetOffline,
    /// <summary>No SteelSeries USB dongle found.</summary>
    DongleNotFound,
}
...
internal sealed class HeadsetService : IDisposable
{
    ...
    /// <summary>
    /// The charge byte (buf[marker+4]) is read directly in <see cref="Parse"/> and is authoritative --
    /// this method is only a trend-based fallback (rising/falling percent) for when
    /// <see cref="Parse"/> returns <see cref="ChargingState.TrendPending"/> (byte missing/unrecognized).
    /// </summary>
    ...
    /// Throws on IO/timeout (handled in <see cref="Poll"/>).
    /// </summary>
    ...
    /// <summary>Parses the report [0xb0, connection, _, battery%, charging] relative to the marker index.</summary>
